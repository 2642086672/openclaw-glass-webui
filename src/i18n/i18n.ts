...
const dict = {
  ...
  usageColSaved: { zh: '估算节省', en: 'Saved' },
  savingsColdInput: { zh: '冷启动输入', en: 'Cold input' },
  savingsHitRate: { zh: '缓存命中率', en: 'Cache hit rate' },
  savingsSaved: { zh: '估算节省', en: 'Est. saved' },
  savingsTipsTitle: { zh: '省钱建议', en: 'Savings tips' },
  savingsTipLow: { zh: '命中率偏低,大量输入按原价计费。', en: 'Low hit rate — most input billed at full price.' },
  savingsTipMid: { zh: '命中率尚可,继续在同一会话里聊还能再省。', en: 'Moderate — keep chatting in the same sessions to save more.' },
  savingsTipHigh: { zh: '命中率良好,系统提示和上下文复用充分。', en: 'Great hit rate — prompts and context are being reused well.' },
  savingsTipContinue: { zh: '在同一个会话里继续聊,而不是频繁开新会话。', en: 'Keep going in existing sessions instead of starting new ones.' },
  savingsTipNoPrice: { zh: '{n} 个模型未配置单价,费用可能低估。去设置→模型管理补上每百万价格。', en: '{n} model(s) missing per-million pricing — cost may be underestimated. Add pricing in Settings → Models.' },
  savingsTipPricing: { zh: '命中率高的关键是:稳定不变的系统提示 + 长会话复用上下文。', en: 'High hit rate comes from stable system prompts and long sessions that reuse context.' },
